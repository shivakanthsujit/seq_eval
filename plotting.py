...
envs = {
    'halfcheetah-random-v2': 999000, 
    'halfcheetah-medium-v2': 999000, 
    'halfcheetah-medium-expert-v2': 1998000, 
    'halfcheetah-medium-replay-v2': 201798, 
    'walker2d-random-v2': 999322, 
    'walker2d-medium-v2': 999322, 
    'walker2d-medium-expert-v2': 1998318, 
    'walker2d-medium-replay-v2': 301698, 
    'hopper-random-v2': 999998, 
    'hopper-medium-v2': 999998, 
    'hopper-medium-expert-v2': 1998966, 
    'hopper-medium-replay-v2': 401598
    }
...
def figure(runs, methods, args):
    ...
    for name, ax in zip(args.add, axes.flatten()[len(tasks) :]):
        ...
        if name == "median":
            plot_combined(
                "combined_median", ax, runs, methods, args, lo="random", hi="human$", agg=lambda x: np.nanmedian(x, -1)
            )
        elif name == "mean":
            plot_combined("combined_mean", ax, runs, methods, args, lo="random", hi="human$", agg=lambda x: np.nanmean(x, -1))
        elif name == "gamer_median":
            plot_combined(
                "combined_gamer_median", ax, runs, methods, args, lo="random", hi="human$", agg=lambda x: np.nanmedian(x, -1)
            )
        elif name == "gamer_mean":
            plot_combined(
                "combined_gamer_mean", ax, runs, methods, args, lo="random", hi="human$", agg=lambda x: np.nanmean(x, -1)
            )
        elif name == "record_mean":
            plot_combined(
                "combined_record_mean", ax, runs, methods, args, lo="random", hi="record", agg=lambda x: np.nanmean(x, -1)
            )
            ...
            plot_combined("combined_num_seeds", ax, runs, methods, args, agg=lambda x: np.isfinite(x).sum(-1))
        elif name == "human_above":
            plot_combined("combined_above_human$", ax, runs, methods, args, agg=lambda y: (y >= 1.0).astype(float).sum(-1))
        elif name == "human_below":
            plot_combined("combined_below_human$", ax, runs, methods, args, agg=lambda y: (y <= 1.0).astype(float).sum(-1))
        else:
    ...
    for ax in axes.flatten()[len(tasks) + len(args.add) :]:
        ax.axis("off")
    legend_opt = {}
    legend_opt.update(LEGEND)
    if args.legend_options:
        legend_opt.update(args.legend_options)
    legend(fig, args.labels, **legend_opt)
    return fig
...
def plot(task, ax, runs, methods, args):
    ...
    if args.ylim:
        ...
        if args.ylimticks:
            ...
            ax.set_yticks(ticks)
    if vline:
        ax.axvline(x = envs[task], color='black', ls='--')
    if mixed:
        ax.axvline(x = 333000, color='black', ls='--')
        ax.axvline(x = 2 * 333000, color='black', ls='--')
...
def plot_combined(name, ax, runs, methods, args, agg, lo=None, hi=None, clip=False):
    ...
    borders = sorted([r.xs for r in runs if r.xs is not None], key=lambda x: np.nanmax(x))[-1]
...
def baseline(index, method, ax, runs, args):
    ...
    if args.mixed:
        ax.axhline(mean, xmin=0, xmax=0.33, label=method, **kw)
        ax.axhline(mean, xmin=0.33, xmax=0.66, label=method, **kw)
        ax.axhline(mean, xmin=0.66, xmax=1, label=method, **kw)
    else:
    ...
    try:
        ...
        for index, line in enumerate(lines):
            try:
                ...
                raise ValueError(f"Skipping invalid JSON line ({index+1}/{len(lines)+1}) in" f"{filename}: {line}")
...
def save_runs(runs, filename):
    ...
    for run in runs:
        if run.xs is None:
            continue
        records.append(dict(task=run.task, method=run.method, seed=run.seed, xs=run.xs.tolist(), ys=run.ys.tolist()))
...
def main(args):
    ...
    runs = load_runs(args)
    if args.save_runs:
        save_runs(runs, args.outdir / f"{args.fname}.jsonl")
...
def parse_args():
    ...
    parser.add_argument("--legend_options", nargs="+", default=None)
    parser.add_argument("--labels", nargs="+", default=None)
    parser.add_argument("--palette", nargs="+", default=["contrast"])
    parser.add_argument("--colors", nargs="+", default={})
    parser.add_argument("--save_runs", type=boolean, default=False)
    parser.add_argument("--plot_baseline", type=float, default=0)
    parser.add_argument("--maxval", type=float, default=0)
    parser.add_argument(
        "--add", nargs="+", type=str, default=["gamer_median", "gamer_mean", "record_mean", "clipped_record_mean", "num_seeds"]
    )
    ...
    if args.legend_options:
        assert len(args.legend_options) % 2 == 0
        legend_options = {}
        for k, v in zip(args.legend_options[:-1: 2], args.legend_options[1: :2]):
            try:
                v = int(v)
            except ValueError:
                pass
            if k == "loc":
                v = v.replace("_", " ")
            legend_options[k] = v
        args.legend_options = legend_options
