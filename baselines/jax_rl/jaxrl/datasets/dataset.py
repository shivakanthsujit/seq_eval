...
class MDLDataset(Dataset):
    ...
    def sample(self, batch_size: int, sample_last: bool = False, obs=0) -> Batch:
        ...
        indx = np.random.choice(self.indices[start: self.insert_index], size=batch_size)
        if sample_last:
            assert obs > 0
            indx[-obs: ] = self.indices[self.insert_index - obs: self.insert_index]
        return Batch(observations=self.observations[indx],
                     actions=self.actions[indx],
                     rewards=self.rewards[indx],
                     masks=self.masks[indx],
