...
if __name__ == "__main__":
    ...
    run_name = os.path.join(args.env, f"TD3BC_{args.id}", f"seed{args.seed}")
    ...
    replay_buffer = utils.MDLReplayBuffer(state_dim, action_dim, mixed=mixed)
    if mixed:
    ...
    evaluations = []
    for t in tqdm(range(1, max_timesteps + 1), miniters=int(2e3)):
        step = t // gsteps * obs
        ...
        # Evaluate episode
        if step % args.eval_freq == 0 and t % gsteps == 0:
            tqdm.write(f"Time steps: {t+1}")
            eval_score = eval_policy(policy, temp_name, args.seed, mean, std)
