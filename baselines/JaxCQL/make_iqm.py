...
ENVS = sorted(
    [
        "halfcheetah-medium-expert-v2", "halfcheetah-medium-replay-v2", "halfcheetah-medium-v2", "hopper-medium-expert-v2", "hopper-medium-replay-v2", "hopper-medium-v2", "walker2d-medium-expert-v2", "walker2d-medium-replay-v2", "walker2d-medium-v2"
    ]
)
...
method="cql"
outdir = f"plots/{method}"
algs = [f"{method}_mdl_strat_freq1-1", f"{method}_mdl_baseline"]

os.makedirs(outdir, exist_ok=True)
seeds = 5

env_scores = {alg: convert_to_matrix(cut_data(data[alg], seeds, ENVS)) for alg in algs}
normalized_env_scores = {alg: scores for alg, scores in env_scores.items()}
...
legend = {
    f"{method}_mdl_baseline": "Normal",
    f"{method}_mdl_strat_freq1-1": "pMDL",
    }
