...
def get_d4rl_dataset(env, mixed = False, game="halfcheetah"):
    if mixed:
        print(f"Loading mixed dataset for {game}")
...
def subsample_mdl_batch(batch, indices, insert_index, size, sample_last=False, obs=0, mixed=False):
    assert insert_index != 0, "Have to set number of datapoints that we can sample from."
    assert insert_index <= batch['observations'].shape[0], "Insert index cant be larger than the dataset."
    start = 0
    if mixed:
        start = max(0, insert_index - 200000)
    chosen_indices = np.random.choice(indices[start: insert_index], size=size)
    if sample_last:
        assert obs > 0
        chosen_indices[-obs: ] = indices[insert_index - obs: insert_index]
    return chosen_indices, index_batch(batch, chosen_indices)
