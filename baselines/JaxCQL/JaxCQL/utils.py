...
class WandBLogger(object):
    ...
    def __init__(self, config, variant):
        ...
        seed = variant["seed"]
        group = os.path.join(variant["env"], self.config.experiment_id)
        if self.config.output_dir == '':
            self.config.output_dir = tempfile.mkdtemp()
        else:
            if "online" in self.config.env_prefix:
                env = variant["env"].split("-")[0]
            else:
                env = variant["env"]
            self.config.output_dir = os.path.join(self.config.output_dir, f"{self.config.env_prefix}{env}", f"cql_{self.config.experiment_id}", f"seed{seed}")
