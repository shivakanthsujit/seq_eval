...
def main(argv):
    ...
    for i in tqdm(range(max_steps)):
        ...
        indx, batch = subsample_mdl_batch(dataset, indices, insert_index, FLAGS.batch_size)
        sampled_indx[indx] += 1
        batch = batch_to_jax(batch)
        metrics.update(prefix_metrics(sac.train(batch, bc=epoch < FLAGS.bc_epochs), 'sac'))

        with Timer() as eval_timer:
            if step % eval_freq == 0 or i == (max_steps - 1):
    ...
    dataset['rewards'] = dataset['rewards'] * FLAGS.reward_scale + FLAGS.reward_bias
    dataset['actions'] = np.clip(dataset['actions'], -FLAGS.clip_action, FLAGS.clip_action)
    replay_buffer = ReplayBuffer(FLAGS.replay_buffer_size, dataset)
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        metrics = {'epoch': step + current_step, "replay_buffer_size": len(replay_buffer)}
