...
class Logger(object):
    ...
    def dump_tabular(self, *args, **kwargs):
        ...
        if len(self._tabular) > 0:
            ...
            if self.writer:
                tabular_dict = {k: float(v) for k, v in tabular_dict.items()}
                epoch = float(tabular_dict["epoch"])
                [self.writer.add_scalar(k, v, epoch) for k, v in tabular_dict.items()]

            metric_file = list(self._tabular_fds.keys())[0]
            metric_file = os.path.split(metric_file)[0]
            f = (Path(metric_file) / "metrics.jsonl").open("a")
            with (Path(metric_file) / "metrics.jsonl").open("a") as f:
                f.write(json.dumps({"step": epoch, **tabular_dict}) + "\n")
...
def setup_logger(
    ...
    logger.writer = SummaryWriter(log_dir, max_queue=50)
