...
class MDLDataset(object):
    def __init__(self, observations: np.ndarray, actions: np.ndarray,
                 rewards: np.ndarray, masks: np.ndarray,
                 dones_float: np.ndarray, next_observations: np.ndarray,
                 size: int, mixed=False):
        self.observations = observations
        self.actions = actions
        self.rewards = rewards
        self.masks = masks
    ...
    def sample(self, batch_size: int, sample_last: bool = False, obs: int = 0) -> Tuple[np.ndarray, Batch]:
        ...
        if sample_last:
            assert obs > 0
            pseudo_indices[-obs:] = self.insert_index - np.arange(1, obs + 1)
        indx = self.indices[pseudo_indices]
        # indx = np.random.choice(self.indices[:self.insert_index], size=batch_size)
        return pseudo_indices, indx, Batch(observations=self.observations[indx],
                     actions=self.actions[indx],
                     rewards=self.rewards[indx],
                     masks=self.masks[indx],
                     next_observations=self.next_observations[indx])

class MDLD4RLDataset(MDLDataset):
    def __init__(self,
                 env: gym.Env,
                 clip_to_eps: bool = True,
                 eps: float = 1e-5,
                 mixed = False, game="halfcheetah"):
        if mixed:
        ...
        super().__init__(dataset['observations'].astype(np.float32),
                         ...
                         next_observations=dataset['next_observations'].astype(
                             np.float32),
                         size=len(dataset['observations']), mixed=mixed)
